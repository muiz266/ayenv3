...
async function resolveShortMapsLink(link) {
  ...
  try {
    ...
    const res = await fetch(s, {
      ...
      headers: {
        "User-Agent": "Mozilla/5.0 (compatible; AyenBot/1.0)",
        "Accept": "text/html,application/xhtml+xml"
      }
    });
    clearTimeout(timer);
    const finalUrl = String(res.url || s);
    // Cuba ekstrak dari URL akhir
    let parsed = parseMapsLink(finalUrl);
    if (parsed.ok) return { ...parsed, link: s.slice(0, 500), resolvedUrl: finalUrl.slice(0, 500) };

    // Fallback: baca body HTML untuk @lat,lng atau !3d !4d
    try {
      const html = await res.text();
      const chunk = String(html || "").slice(0, 200000);
      let lat = null, lng = null;
    ...
    return {
      ok: false,
      error: "Pautan pendek tidak dapat dibaca. Cuba: buka pautan di pelayar → Kongsi semula → salin pautan penuh (ada @lat,lng)."
    };
  } catch (e) {
    return {
      ok: false,
      error: "Gagal baca pautan pendek Google Maps. Sila salin pautan penuh dari Kongsi → Salin pautan."
    };
  }
}
...
app.post("/api/maps/resolve", async (req, res) => {
  try {
    ...
    console.error(e);
    res.status(500).json({ ok: false, error: "Gagal semak pautan Maps." });
  }
});
...
app.post("/api/orders/:orderId/rider-proof", upload.single("photo"), async (req, res) => {
  try {
    ...
    if (status === "Dalam penghantaran") {
      ...
      await pool.query(
        `UPDATE orders SET status=$1, delivery_photo_data=$2, delivery_photo_type=$3,
          delivery_lat=$4, delivery_lng=$5 WHERE order_id=$6`,
    ...
    try {
      ...
      let autoText = "";
      if (status === "Dalam penghantaran") {
        autoText = "📷 [BUKTI_PICKUP] Pesanan telah di-pickup di kedai. Rider sedang menuju kepada anda.";
      } else if (status === "Selesai") {
        autoText = "📷 [BUKTI_HANTAR] Pesanan telah dihantar. Terima kasih!";
        if (lat != null && lng != null) {
          autoText += " Lokasi: https://maps.google.com/?q=" + lat + "," + lng;
        }
      }
      if (autoText) {
        await pool.query(
    ...
    try { await notifyAllParties(id, { event: "order_update", body: "Pesanan " + id + ": " + status }); } catch (_) {}
  ...
  try {
    ...
    res.json({
      messages: r.rows.map((m) => {
        ...
        let image = null;
        if (text.includes("[BUKTI_PICKUP]") && pickupData) {
          image = "data:" + pickupType + ";base64," + pickupData;
        } else if (text.includes("[BUKTI_HANTAR]") && deliveryData) {
