...
function parseMapsLink(link) {
  ...
  if (!/^https?:\/\//i.test(s)) return { ok: false, error: "Pautan Maps mesti bermula dengan http(s)." };
  ...
  if (lat != null && lng != null && Math.abs(lat) <= 90 && Math.abs(lng) <= 180) {
    return { ok: true, link: s.slice(0, 500), lat, lng };
  }
  return { ok: true, link: s.slice(0, 500), lat: null, lng: null };
}
...
app.post("/api/orders", upload.single("receipt"), async (req, res) => {
  ...
  try {
    ...
    const mapsParsed = parseMapsLink(payload.mapsLink || payload.maps_link || "");
